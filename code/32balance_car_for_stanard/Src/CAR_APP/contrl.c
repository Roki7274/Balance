...
int   Dead_Zone=3200;    //电机死区3100
int   control_turn=0;                             //转向控制
//1.5  100  0.1
extern int Turn_Pwm;
//PID调节参数
struct pid_arg PID = {
	.Balance_Kp=300 ,//-230 245  -300  -240
	.Balance_Kd=1.80 ,//-0.16 0.50  -1.50 -1.90
	.Velocity_Kp=200,//-48 70 100   200 260
	.Velocity_Ki=1.4,//-0.24 0.21 0.325  0.08 1.5
	.Turn_Kp = 70,
	.Turn_Kd = 0.5,//0.5
};
/**************************************************************************************************************
...
{
	static float Velocity,Encoder_Least,Encoder=0.0f;
	static float Encoder_Integral=0.0f;
	Encoder_Least =(encoder_left+encoder_right)/2.0-Movement;    //获取最新速度偏差=测量速度（左右编码器之和）-目标速度（此处为零）
...
////	
	...
	if(	Velocity<-6000) 	Velocity=-6000;
//	Velocity=Encoder*PID.Velocity_Kp;      //速度控制
...
*返回值:无
***************************************************************************************************************/
int Vertical_turn_PD(int CCD,short yaw)
{
		float Turn;     
    float Bias;	 
//    float Bias_err;	
//		float last_Bias;
	  Bias=CCD;
//	  Bias_err=Bias-last_Bias;
//	  last_Bias=Bias;
	  Turn=Bias*PID.Turn_Kp+yaw*PID.Turn_Kd;
//	Turn=Bias*PID.Turn_Kp+Bias_err*PID.Turn_Kd;
	  return Turn;
}

//int Vertical_turn_PD(*angle,short yaw)
//{
//		float Turn;     
//    float Bias;	  
//	  Bias=angle-zhongzhi;
...
{   
	...
	if(motor1>0)
{			
	PWMA2=Dead_Zone+(abs(motor1))*1.17;//1.17
	PWMA1=0;
...
float straight(float x,float * straight_speed)
{
  * straight_speed=-x/2.4;
}

float stop(float * straight_speed)
{
...
float distance(float * straight_speed)
{
if (BT_Data.resulte[4]>95)
{
   *straight_speed=0.0f;
}

}
void  follow_inner(int  Contrl_Turn)
{  
	if( BT_Data.resulte[0]>90||BT_Data.resulte[2]>160)  //140
	{
	  Contrl_Turn=BT_Data.resulte[0]-90;//左
	}	
	else if(BT_Data.resulte[0]<90||BT_Data.resulte[2]<160)
	{
 		Contrl_Turn =BT_Data.resulte[0]-90;		
//	 Contrl_Turn=0;
	}
	if(BT_Data.resulte[0]<70||BT_Data.resulte[0]>120)
	{
	 Contrl_Turn=0;
	}

}
void  follow_outer(int  Contrl_Turn)
{  
	if( BT_Data.resulte[1]>95||BT_Data.resulte[2]>160)  //140
	{
	  Contrl_Turn=BT_Data.resulte[0]-95;//左
	}	
	else if(BT_Data.resulte[1]<95||BT_Data.resulte[2]<160)
	{
 		Contrl_Turn =BT_Data.resulte[0]-95;		
//	 Contrl_Turn=0;
	}
	if(BT_Data.resulte[1]<70||BT_Data.resulte[1]>120)
	{
	 Contrl_Turn=0;
	}
